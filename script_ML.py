# -*- coding: utf-8 -*-
"""
Analytics and Machine Learning Pipeline for Prediction of Early Adopters of T2D Drugs
...
#%% DEFINE CLASSES AND GENERATE CLASS INDEX HERE
innovator_index = doc_info_frame['labels'] == 1     # Innovator Class indices
traditional_index = doc_info_frame['labels'] == 0   # Traditional Class indices

classes_dict = {'Innovators': list(innovator_index), 'Traditional': list(traditional_index)}
class_labels = classes_dict.keys()

# importing feature visualization functions
from func_data_exploration import plot_stacked_histogram, plot_feature_histograms
# visualize features
plot_feature_histograms(doc_info_frame, ['percent_pov'], classes_dict, class_labels, 'Percent Poverty')

#%% Extracting the exact features for use
classes=list(classes_dict.keys())

features_combination=doc_info_frame[['a_state', 'b_enumeration_date', 'b_gender', 'b_sole_proprietor', 'i_desc', 't_desc', 't_primary','percent_pov_cate']]
#features_combination=doc_info_frame[['a_state', 'b_enumeration_date', 'b_gender', 'b_sole_proprietor', 'i_desc', 't_primary','percent_pov_cate']]

features=pd.get_dummies(features_combination)
featuresEncoded=features.as_matrix()

#% Building the classifier -> Gradient Boosting
from sklearn.model_selection import train_test_split
from sklearn.ensemble import GradientBoostingClassifier

X,y=featuresEncoded,doc_info_frame['labels'].as_matrix()
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20, random_state=10) # Change random state to vary test set

#%% Random search for GradBoost
...
# parameter range for randomized hyper-parameter search
param_dist = {"max_depth": [3, None],
              "max_features": sp_randint(1, featuresEncoded.shape[1]),
...
n_iter_search=100 # number of hyper-parameters to search

clf = RandomizedSearchCV(GradientBoostingClassifier(n_estimators=n_estimators), 
                                   param_distributions=param_dist, 
                                   n_iter=n_iter_search, 
                                   scoring=scoring, 
                                   cv=3) # 3 fold cross validation in random grid search
...
# Utility function to report the best 3 scores
...
#%% Now moving on to the test dataset - confusion matrix, AUC curve, cross-validated prediction
from sklearn.metrics import classification_report, accuracy_score
from sklearn.metrics import confusion_matrix
from func_plot_confusion_matrix import plot_confusion_matrix
...
cnf_matrix = confusion_matrix(y_test, y_pred) # no cross validation here, that's why such high numbers
...
plot_confusion_matrix(cnf_matrix, classes=classes, normalize=True, title='Normalized confusion matrix') ### Test Recall
plt.show()

# Printing more detailed report on the classification measures
print(classification_report(y_test, y_pred, target_names=classes))
accuracy_score(y_test, y_pred)

# Uncomment to plot non-normalized confusion matrix
#plt.figure()
...
# Compute ROC curve and the area under curve
fpr, tpr, thresholds = roc_curve(y_test, probas_[:, 1]) # probability for the positive class

roc_auc = auc(fpr, tpr)
#plt.plot(fpr, tpr, lw=1, alpha=0.3, label='ROC fold %d (AUC = %0.2f)' % (i, roc_auc))

plt.figure(figsize=(8,6))    
plt.plot([0, 1], [0, 1], linestyle='--', lw=2, color='g', label='Chance', alpha=.8)

plt.plot(fpr, tpr, color='b', label=r'AUC = %0.2f ' % (roc_auc), lw=3, alpha=.8)
...
#%%
### Running a separate cross validation check
from sklearn.model_selection import cross_val_score
scores = cross_val_score(clf, X_train, y_train, cv=3, scoring=scoring)
print("Crossval Training Recall: %0.3f (+/- %0.3f)" % (scores.mean(), scores.std()*2))

#%% ROC plotting with the full data stratified into 6 splits  
stratifiedFolds = StratifiedKFold(n_splits=6)
tprs = []
aucs = []
...
i = 0
plt.figure(figsize=(8,6))
for train, test in stratifiedFolds.split(X, y):
    probas_ = clf.fit(X[train], y[train]).predict_proba(X[test])
    # Compute ROC curve and the area under curve
    fpr, tpr, thresholds = roc_curve(y[test], probas_[:, 1]) # probability for the positive class
...
plt.figure(figsize=(8,6))    
plt.plot([0, 1], [0, 1], linestyle='--', lw=2, color='g', label='Luck', alpha=.8)

mean_tpr = np.mean(tprs, axis=0)
mean_tpr[-1] = 1.0
mean_auc = auc(mean_fpr, mean_tpr)
std_auc = np.std(aucs)
plt.plot(mean_fpr, mean_tpr, color='b', label=r'Mean (AUC = %0.2f $\pm$ %0.2f)' % (mean_auc, std_auc), lw=3, alpha=.8)

std_tpr = np.std(tprs, axis=0)
tprs_upper = np.minimum(mean_tpr + std_tpr, 1)
tprs_lower = np.maximum(mean_tpr - std_tpr, 0)
plt.fill_between(mean_fpr, tprs_lower, tprs_upper, color='grey', alpha=.2, label=r'$\pm$ 1 SD')
...
# Plotting various interesting data
